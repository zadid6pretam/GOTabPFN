...
# NSC.py
#
# Original NSC: descriptor/statistics-based Neuro-Inspired
# Subunit Compression
#
# Core class:
#   NSC
...
#       --descriptor basic \
#       --pooling learn_free \
#       --out-prefix colon_nsc
#
# Python/package example:
#   from gotabpfn import run_nsc_csv
#
#   result = run_nsc_csv(
...
#   result["compressed_df"].to_csv("colon_nsc.csv", index=False)
# ============================================================
...
# ============================================================
# Helper: Estimate Intrinsic Dimensionality
# Kept from your implementation.
# ============================================================

def estimate_intrinsic_dim_effective_rank(X: np.ndarray, eps: float = 1e-8) -> float:
    """
    X: (n, m) numpy array (float)
    Returns: d_eff (effective rank) as intrinsic dimension estimate.
    Uses eigenvalues of Gram matrix (n x n), cheap for HDLSS.
    """
    X = X.astype(np.float64, copy=False)
    # standardize per feature
    Xc = X - X.mean(axis=0, keepdims=True)
    Xc /= (Xc.std(axis=0, keepdims=True) + eps)

    n, m = Xc.shape
    # Gram matrix (n x n)
    Gram = (Xc @ Xc.T) / float(max(n - 1, 1))
    ...
    p = evals / evals.sum()
    d_eff = float(np.exp(-np.sum(p * np.log(p + eps))))
    return d_eff
...
def run_nsc_csv(
    ...
    """
    Run original NSC compression on a CSV dataset.

    Original NSC does not estimate PCA-IDF internally. For convenience,
    this wrapper can estimate d_hat using effective rank if estimate_d_hat=True.
    ...
    if d_hat is None and M is None:
        if estimate_d_hat:
            d_hat = estimate_intrinsic_dim_effective_rank(X)
        else:
            raise ValueError(
                "Original NSC requires either d_hat or M. "
                "Use d_hat=..., M=..., or estimate_d_hat=True."
            )
    ...
    print(f"[NSC] Dataset: {dataset_name}")
    print(f"[NSC] X shape: {X.shape}")
    print(f"[NSC] ordering: {'identity' if ordering_csv is None else ordering_csv}")
    print(
        f"[NSC] segmentation={segmentation}, m_rule={m_rule}, "
        f"descriptor={descriptor}, pooling={pooling}, d_hat={d_hat}, M={M}"
    )
    ...
    metrics = {
        ...
        "pooling": pooling,
        "m_rule": m_rule,
        "d_hat_given_or_estimated": None if d_hat is None else float(d_hat),
        "d_hat_source": "effective_rank" if estimate_d_hat and M is None else ("provided" if d_hat is not None else "not_used"),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run original NSC compression on a numeric CSV dataset."
    )
    ...
    parser.add_argument("--estimate-d-hat", action="store_true")
    ...
    run_nsc_csv(
        ...
        d_hat=args.d_hat,
        M=args.M,
        estimate_d_hat=args.estimate_d_hat,
        gamma=args.gamma,
        M_min=args.M_min,
        M_max=args.M_max,
